    ...
    for key, index in mapping.items():
        if directorio.endswith(key):
            if index < len(template_names):
                return os.path.join(template_path, template_names[index])
            else:
...
def crear_graficas_anuales(ws, fila_encabezado, col_anio):
    ...
    # Recolecta las filas en las que hay dato en la columna de referencia
    ...
    # Se cuenta las columnas con dato en la primera fila de datos
    valid_cols = 0
    for col in range(col_anio, ws.max_column + 1):
        if ws.cell(row=fila_data, column=col).value is not None:
            valid_cols += 1

    # Si solo existe la columna de referencia se usa esa, de lo contrario,
    # se utiliza la columna siguiente para los valores a graficar
    if valid_cols == 1:
    ...
    # Formateo del eje Y y asignación de título, con lógica de escalado
    max_val = None
    for row in ws.iter_rows(min_row=graf_start, max_row=graf_end,
                            min_col=(col_anio if valid_cols == 1 else col_anio+1),
                            max_col=(col_anio if valid_cols == 1 else col_anio+1)):
        for cell in row:
            try:
